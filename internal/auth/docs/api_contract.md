...
>
> Cả hai endpoint Prep (`/auth/api/v1.1/auth/me` và `/api/v1.1/auth/token`) đều dùng chung `PREP_AUTH_SERVICE_DOMAIN`.
...
2. Gọi `PrepUserService.ValidateToken()` — check Redis cache → miss thì gọi Prep `{PREP_AUTH_SERVICE_DOMAIN}/auth/api/v1.1/auth/me` (qua circuit breaker)
